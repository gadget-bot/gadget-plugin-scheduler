...
EXECUTABLE  := gadget-plugin-scheduler
...
container: ## Build container using docker
	@$(MAKE) --no-print-directory log-$@
	@docker build -t gadget-plugin-scheduler:local .
...
start-db: ## Start maria db - export DB_ROOT_PASS and DB_PASS to set credentials
	@$(MAKE) --no-print-directory log-$@
	@docker run --name gadget-plugin-scheduler-mariadb \
		-v ${HOME}/.gadget-plugin-scheduler/db:/var/lib/mysql \
		-e MARIADB_ROOT_PASSWORD="${DB_ROOT_PASS}" \
		-e MARIADB_DATABASE=gadget_plugin_scheduler_dev \
...
stop-db: ## Stop maria db
	@$(MAKE) --no-print-directory log-$@
	@docker stop gadget-plugin-scheduler-mariadb
	@docker rm gadget-plugin-scheduler-mariadb
